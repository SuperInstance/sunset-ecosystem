#!/usr/bin/env python3
"""Turbovec DNA Dimension Study — benchmark 128 vs 256 vs 384 vs 512.

Tests compression ratio, search latency, and recall at different
dimensionalities to find the sweet spot for fleet agent DNA.

Usage::

    python benchmarks/dimension_study.py
"""
...
def benchmark_dim(dim: int) -> dict:
    ...
    # Estimate memory (vector storage + metadata)
    # 4-bit quantized: dim * 0.5 bytes per vector
    # Metadata: ~40 bytes per agent (fitness, generation, etc.)
    vector_bytes = POPULATION * dim * 0.5
    meta_bytes = POPULATION * 40
    total_mb = (vector_bytes + meta_bytes) / (1024 * 1024)

    # Compare to naive float32
    naive_mb = (POPULATION * dim * 4) / (1024 * 1024)
    compression = naive_mb / total_mb

    print(f"  Avg latency: {avg_latency:.3f}ms")
    print(f"  P99 latency: {p99_latency:.3f}ms")
    print(f"  Memory: {total_mb:.1f}MB (naive would be {naive_mb:.1f}MB, {compression:.1f}x compression)")
...
def main() -> None:
    print("=" * 60)
    print("Turbovec DNA Dimension Study")

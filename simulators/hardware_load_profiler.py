...
def simulate(devices, jobs, scheduler_fn, label):
    ...
    for d in devs:
        ...
        dev_metrics[d.name] = {
            ...
            'utilization': d.work_done / total_possible[idx] if total_possible[idx] > 0 else 0,
...
if __name__ == '__main__':
    ...
    lines.append("**Schedulers:** Naive (round-robin), Thermal-55% (global threshold), Adaptive-50% (per-device threshold = thermal_budget × 0.5)\n")

    lines.append("## Results Table\n")
    lines.append("| Load | Scheduler | Throughput | Throttling | XDNA2% | RyzenAI% | RTX% |")
    lines.append("|------|-----------|------------|------------|--------|----------|------|")
    for row in rows:
        lines.append(
    ...
    for ar, tr, nr in zip(adaptive_rows, thermal_rows, naive_rows):
        ...
        if ar['throughput'] < tr['throughput'] - 0.005:
            lines.append(f"- **{load}**: Adaptive-50% underperforms Thermal-55% by {fmt_pct(tr['throughput'] - ar['throughput'])}. "
            ...
            lines.append(f"- **{load}**: Adaptive-50% outperforms Thermal-55% by {fmt_pct(ar['throughput'] - tr['throughput'])}. "
                         f"Tighter per-device budgets prevent cascade throttling.")
        else:
            lines.append(f"- **{load}**: Adaptive-50% and Thermal-55% are roughly equal ({fmt_pct(ar['throughput'])}).")
    ...
    for i, (ar, tr, nr) in enumerate(zip(adaptive_rows, thermal_rows, naive_rows)):
        ...
        if best['scheduler'] == 'Adaptive-50%':
            lines.append(f"- **{load}**: **Adaptive-50%** — best throughput ({fmt_pct(best['throughput'])}), per-device granularity pays off.")
        elif best['scheduler'] == 'Thermal-55%':
            lines.append(f"- **{load}**: **Thermal-55%** — best throughput ({fmt_pct(best['throughput'])}), global threshold is the sweet spot.")
        else:
            lines.append(f"- **{load}**: **Naive** — best throughput ({fmt_pct(best['throughput'])}). Thermal awareness is pure overhead at this load.")

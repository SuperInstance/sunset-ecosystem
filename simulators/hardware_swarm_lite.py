...
if __name__ == "__main__":
    ...
    for load in loads:
        if adaptive_by_load[load] and thermal_by_load[load]:
            if (
                adaptive_by_load[load]["throughput"]
                < thermal_by_load[load]["throughput"]
            ):
                breakdown_load = load
                break
    ...
    for r in results:
        ...
        sched = parts[1]
        md.append(
            f"| {load} | {sched} | {r['throughput']:.2%} | {r['throttle_steps']} | "
    ...
    if breakdown_load:
        ...
        md.append(
            f"Adaptive thresholding loses its advantage at {breakdown_load:.0%} load. "
        ...
        md.append(
            "Adaptive maintained advantage across all tested load levels (50%–95%). "
    ...
    for load in loads:
        candidates = [
            ...
            (
                "Thermal",
                thermal_by_load[load]["throughput"] if thermal_by_load[load] else 0,
            ),
            (
                "Adaptive",
                adaptive_by_load[load]["throughput"] if adaptive_by_load[load] else 0,
            ),
        ]
    ...
    for r in results:
        ...
        md.append(
            f"| {load} | {sched} | {td.get('RTX4050', 0)} | {td.get('RyzenAI', 0)} | "

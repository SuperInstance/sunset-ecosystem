...
def _compiled_forward_max(w, x):
    ...
    return _forward_einsum_numba_max(xflat, w["w1"], b1, w["w2"], b2, w["w3"], b3, n)
...
for n in [100, 250, 500, 1000]:
    ...
    _compiled_forward_max(w, x)  # warmup
    forward_einsum(w, x)  # warmup
    trials = 50
    t0 = time.perf_counter()
    for _ in range(trials):
        _compiled_forward_max(w, x)
    t_numba = (time.perf_counter() - t0) * 1000
    t0 = time.perf_counter()
    for _ in range(trials):
        forward_einsum(w, x)
    t_numpy = (time.perf_counter() - t0) * 1000
    print(f"n={n:4d}  numba_max={t_numba/trials:.3f}ms  numpy={t_numpy/trials:.3f}ms  speedup={t_numpy/t_numba:.2f}x")
    expected = forward_einsum(w, x)
    actual = _compiled_forward_max(w, x)

...
def _forward_einsum_numba_reordered(
    ...
    for i in prange(n):
        ...
        for j in range(32):
            ...
            if h[j] < 0.0:
                h[j] = 0.0
        h2 = np.zeros(16, dtype=np.float32)
        for k in range(32):
            hk = h[k]
            for j in range(16):
                h2[j] += hk * w2[i, k, j]
...
def _compiled_forward_reordered(w, x):
    ...
    return _forward_einsum_numba_reordered(xflat, w["w1"], b1, w["w2"], b2, w["w3"], b3, n)
...
for n in [100, 250, 500, 1000]:
    ...
    _compiled_forward_reordered(w, x)  # warmup
    forward_einsum(w, x)  # warmup
    trials = 50
    t0 = time.perf_counter()
    for _ in range(trials):
        _compiled_forward_reordered(w, x)
    t_numba = (time.perf_counter() - t0) * 1000
    t0 = time.perf_counter()
    for _ in range(trials):
        forward_einsum(w, x)
    t_numpy = (time.perf_counter() - t0) * 1000
    print(f"n={n:4d}  numba_reordered={t_numba/trials:.3f}ms  numpy={t_numpy/trials:.3f}ms  speedup={t_numpy/t_numba:.2f}x")
    expected = forward_einsum(w, x)
    actual = _compiled_forward_reordered(w, x)

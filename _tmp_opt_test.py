...
def forward_optimized(w, x):
    x = x.ravel().astype(np.float32)
    n = w["w1"].shape[0]
    # h = x @ w1 + b1  → (n, 32)
    h = np.dot(w["w1"], x)  # w1 is (n, 64, 32), dot with (64,) → (n, 32)
    # wait, np.dot((n,64,32), (64,)) gives (n, 32) if last dim of w1 matches x
    h = np.dot(w["w1"], x) + w["b1"][0]
    np.maximum(h, 0, out=h)
    # h @ w2 + b2 → (n, 16)
    # w2 is (n, 32, 16), h is (n, 32)
    # We want h[i] @ w2[i] for each i
    # Can use einsum or batched matmul
    h2 = np.einsum("nh,nhl->nl", h, w["w2"], optimize=True) + w["b2"][0]
    np.maximum(h2, 0, out=h2)
    out = np.einsum("nl,nll->nl", h2, w["w3"], optimize=True) + w["b3"][0]
    return out
...
for n in [100, 250, 500, 1000]:
    ...
    t0 = time.perf_counter()
    for _ in range(trials):
        forward_optimized(w, x)
    t_opt = (time.perf_counter() - t0) * 1000

    print(f"n={n:4d}  orig={t_orig/trials:.3f}ms  opt={t_opt/trials:.3f}ms  speedup={t_orig/t_opt:.2f}x")
    # correctness
    expected = forward_einsum(w, x)
    actual = forward_optimized(w, x)

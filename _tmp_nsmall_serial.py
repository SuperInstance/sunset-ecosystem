...
def _forward_einsum_numba_serial(
...
def _compiled_forward_serial(w, x):
    ...
    return _forward_einsum_numba_serial(xflat, w["w1"], b1, w["w2"], b2, w["w3"], b3, n)
...
for n in [1, 5, 10, 20, 50]:
    ...
    _compiled_forward_serial(w, x)  # warmup
    forward_einsum(w, x)  # warmup
    trials = 100
    t0 = time.perf_counter()
    for _ in range(trials):
        _compiled_forward_serial(w, x)

...
def forward_matmul2(w, x):
    x = x.ravel().astype(np.float32)
    # x[None,:] is (1,64), w1 is (n,64,32)
    # np.matmul broadcasts: (1,64) @ (n,64,32) -> (n, 32)
    h = np.matmul(x[np.newaxis, :], w["w1"]) + w["b1"][0]
...
for n in [100, 250, 500, 1000]:
    ...
    actual = forward_matmul2(w, x)
    diff = np.max(np.abs(expected - actual))
    if diff > 1e-5:
        print(f"n={n} diff={diff:.2e} — SKIP")
        continue
    trials = 50

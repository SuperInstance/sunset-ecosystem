...
class ConsumerGroup:
    ...
    name: str
    offsets: dict[str, int] = field(default_factory=dict)  # topic -> last consumed offset


class EventStream:
    """Persistent event stream with replay and consumer groups."""

    def __init__(self, max_events: int = 10_000, ttl_seconds: float | None = None) -> None:
    ...
    def latest(self, topic: str | None = None) -> Event | None:
        ...
        events = self._events if topic is None else [e for e in self._events if e.topic == topic]
    ...
    def consume(self, group_name: str, topic: str = "default", limit: int = 100) -> list[Event]:
    ...
    def reset_consumer_offset(self, group_name: str, topic: str = "default", offset: int = 0) -> None:

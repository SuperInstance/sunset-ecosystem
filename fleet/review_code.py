...
class ReviewFindings:
    ...
    def add(self, severity: str, message: str, line: int, col: int = 0) -> None:
        self.findings.append(ReviewFinding(
            persona=self.persona, severity=severity, message=message, line=line, col=col
        ))
...
class ReviewPersona(Enum):
    ...
    def all(cls) -> List["ReviewPersona"]:
        return [cls.CORRECTNESS, cls.SECURITY, cls.PERFORMANCE, cls.SIMPLICITY, cls.ADVERSARIAL]
...
class CorrectnessVisitor(PersonaVisitor):
    ...
    def visit_ExceptHandler(self, node: ast.ExceptHandler) -> None:
        if node.type is None:
            self._findings.add("warning", "Bare except clause catches everything including KeyboardInterrupt", node.lineno)
        elif isinstance(node.type, ast.Name) and node.type.id == "Exception":
            self._findings.add("info", "Catching generic Exception — consider more specific types", node.lineno)
        self.generic_visit(node)
        ...
        for op in node.ops:
            if isinstance(op, ast.Is):
                if isinstance(node.left, ast.Constant):
                    self._findings.add("warning", "Using 'is' with literal — use '==' instead", node.lineno)
        self.generic_visit(node)
    ...
    def visit_Call(self, node: ast.Call) -> None:
        ...
        if isinstance(node.func, ast.Name) and node.func.id == "dict":
            if not node.args and not node.keywords:
                self._findings.add("info", "dict() call — could be {} for clarity", node.lineno)
        self.generic_visit(node)
    ...
    def visit_FunctionDef(self, node: ast.FunctionDef) -> None:
        ...
        for default in node.args.defaults:
            if isinstance(default, (ast.List, ast.Dict, ast.Set)):
                self._findings.add("critical", "Mutable default argument — use None and initialize in body", node.lineno)
        super().visit_FunctionDef(node)

    def visit_Return(self, node: ast.Return) -> None:
        if node.value is None:
            self._findings.add("info", "Bare return — ensure all paths return consistent types", node.lineno)
        self.generic_visit(node)
...
class SecurityVisitor(PersonaVisitor):
    ...
    SUSPICIOUS_PATTERNS = [
        (re.compile(r'password\s*=\s*["\'][^"\']+["\']'), "Hardcoded password detected"),
    ...
    def visit_Call(self, node: ast.Call) -> None:
        if isinstance(node.func, ast.Name):
            if node.func.id in self.DANGEROUS_BUILTINS:
                self._findings.add("critical", f"Dangerous builtin '{node.func.id}' — major security risk", node.lineno)
            if node.func.id == "input" and self._loop_depth > 0:
                self._findings.add("warning", "input() inside loop — potential for injection or DoS", node.lineno)
        self.generic_visit(node)
        ...
        if isinstance(node.op, ast.Mod):
            if isinstance(node.left, ast.Constant) and isinstance(node.left.value, str):
                if "SELECT" in node.left.value.upper() or "INSERT" in node.left.value.upper():
                    self._findings.add("critical", "SQL string formatting — SQL injection risk", node.lineno)
        if isinstance(node.op, ast.Add):
            if isinstance(node.left, ast.Constant) and isinstance(node.left.value, str):
                if "SELECT" in node.left.value.upper():
                    self._findings.add("critical", "SQL string concatenation — SQL injection risk", node.lineno)
        self.generic_visit(node)
        ...
        for value in node.values:
            if isinstance(value, ast.Constant) and isinstance(value.value, str):
                if "SELECT" in value.value.upper() or "INSERT" in value.value.upper():
                    self._findings.add("critical", "SQL in f-string — SQL injection risk", node.lineno)
        self.generic_visit(node)

    def visit_ImportFrom(self, node: ast.ImportFrom) -> None:
        if node.module == "pickle":
            self._findings.add("warning", "Pickle import — deserialization risk if loading untrusted data", node.lineno)
        self.generic_visit(node)
...
class PerformanceVisitor(PersonaVisitor):
    ...
    def visit_For(self, node: ast.For) -> None:
        ...
        if self._loop_depth >= 2:
            self._findings.add("warning", f"Nested loop at depth {self._loop_depth} — potential O(n^{self._loop_depth})", node.lineno)
        # Detect list comprehension inside loop (double iteration)
        for child in ast.walk(node):
            if isinstance(child, ast.ListComp) and child is not node:
                self._findings.add("warning", "List comprehension inside loop — consider extracting", node.lineno)
                break
        self.generic_visit(node)
        self._loop_depth -= 1
    ...
    def visit_ListComp(self, node: ast.ListComp) -> None:
        ...
        if len(generators) > 2:
            self._findings.add("info", f"List comprehension with {len(generators)} generators — consider explicit loop", node.lineno)
        self.generic_visit(node)
    ...
    def visit_FunctionDef(self, node: ast.FunctionDef) -> None:
        ...
        for func_name, count in calls.items():
            if count > 5:
                self._findings.add("info", f"Function '{func_name}' called {count} times — consider caching", node.lineno)
        super().visit_FunctionDef(node)
...
class SimplicityVisitor(PersonaVisitor):
    ...
    def visit_FunctionDef(self, node: ast.FunctionDef) -> None:
        ...
        if line_count > 50:
            self._findings.add("warning", f"Function is {line_count} lines — consider decomposition", node.lineno)
        if line_count > 100:
            self._findings.add("critical", f"Function is {line_count} lines — strongly consider decomposition", node.lineno)
        # Check parameter count
        param_count = len(node.args.args) + len(node.args.kwonlyargs)
        if param_count > 7:
            self._findings.add("warning", f"Function has {param_count} parameters — consider a dataclass/config object", node.lineno)
        super().visit_FunctionDef(node)
    ...
    def visit_ClassDef(self, node: ast.ClassDef) -> None:
        ...
        if methods and len(static_methods) == len(methods):
            self._findings.add("info", "Class with only static methods — consider using a module", node.lineno)
        # Check for empty classes
        if not methods and not any(isinstance(n, ast.Assign) for n in node.body):
            self._findings.add("warning", "Empty class — consider if it's needed", node.lineno)
        self.generic_visit(node)
    ...
    def visit_If(self, node: ast.If) -> None:
        ...
        depth = 0
        current = node
        while isinstance(current, ast.If) and isinstance(current.orelse, list) and len(current.orelse) == 1 and isinstance(current.orelse[0], ast.If):
            depth += 1
            current = current.orelse[0]
        if depth > 3:
            self._findings.add("warning", f"Deep if-elif chain ({depth+1} levels) — consider match/dispatch table", node.lineno)
        self.generic_visit(node)
    ...
    def visit_While(self, node: ast.While) -> None:
        ...
        if isinstance(node.test, ast.Constant) and node.test.value is True:
            if not any(isinstance(n, ast.Break) for n in ast.walk(node)):
                self._findings.add("critical", "while True with no break — infinite loop", node.lineno)
        self.generic_visit(node)

    def visit_ExceptHandler(self, node: ast.ExceptHandler) -> None:
        # Check for pass in except
        if len(node.body) == 1 and isinstance(node.body[0], ast.Pass):
            self._findings.add("warning", "Bare pass in except handler — silently swallowing errors", node.lineno)
        self.generic_visit(node)
...
class AdversarialVisitor(PersonaVisitor):
    ...
    def visit_FunctionDef(self, node: ast.FunctionDef) -> None:
        ...
        if not has_validation and len(node.args.args) > 0:
            self._findings.add("warning", "No input validation detected — consider adversarial inputs", node.lineno)
        ...
        for child in ast.walk(node):
            if isinstance(child, ast.BinOp) and isinstance(child.op, (ast.Div, ast.FloorDiv, ast.Mod)):
                ...
                if not isinstance(child.right, ast.Constant):
                    self._findings.add("warning", "Division by variable without zero check — ZeroDivisionError risk", child.lineno)
        super().visit_FunctionDef(node)
    ...
    def visit_Subscript(self, node: ast.Subscript) -> None:
        ...
        if isinstance(node.slice, ast.Constant):
            if isinstance(node.slice.value, int) and node.slice.value < 0:
                self._findings.add("info", "Negative index access — ensure list is long enough", node.lineno)
    ...
    def visit_Call(self, node: ast.Call) -> None:
        ...
        if isinstance(node.func, ast.Name) and self._function_depth > 0:
            self._findings.add("info", "Recursive or nested function call — ensure base case is reachable", node.lineno)
        self.generic_visit(node)
    ...
    def run(self, source: str) -> ReviewFindings:
        ...
        if not has_all and any(isinstance(n, (ast.FunctionDef, ast.ClassDef)) for n in tree.body):

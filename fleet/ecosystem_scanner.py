...
class EcosystemScanner:
    ...
    KNOWN_REPOS = [
        ...
        (
            "stable-worldmodel",
            "World model for spatial reasoning",
            ["python", "rl", "world-model"],
        ),
        (
            "cocapn-health",
            "Fleet health monitoring",
            ["python", "monitoring", "health"],
        ),
    ...
    def find_compatible_repos(
        self, repos: Optional[List[RepositoryInfo]] = None, threshold: float = 0.5
    ) -> List[RepositoryInfo]:
    ...
    def export_markdown(self, filename: str = "ECOSYSTEM_SCAN.md"):
        ...
        for rec in data["recommended_priority"]:
            lines.append(
                f"1. **{rec['repo']}** -> `{rec['module']}` (score: {rec['score']:.2f})"
            )
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            "total_repos": len(repos),
            "compatible_repos": len(compatible),
            "avg_compatibility": np.mean([r.compatibility for r in compatible])
            if compatible
            else 0,

...
class BreederFactory:
    ...
    @classmethod
    def register(
        cls, breeder_type: str, constructor: Callable[[ConstructManifest], Any]
    ) -> None:
    ...
    def create(cls, manifest: ConstructManifest) -> Any:
        if manifest.breeder_type not in cls._registry:
            raise ValueError(
                f"Unknown breeder type: {manifest.breeder_type}. "
...
class ValidationGate:
    ...
    def __init__(
        self, name: str, check_fn: Callable[[Any], Tuple[bool, str]], hard: bool = True
    ):
        self.name = name
...
GATE_REGISTRY: Dict[str, ValidationGate] = {
    "exact_arithmetic": ValidationGate(
        "exact_arithmetic", exact_arithmetic_gate, hard=True
    ),
    "holonomic_consistency": ValidationGate(
        "holonomic_consistency", holonomic_consistency_gate, hard=True
    ),
...
class ProgressStreamer:
    ...
    def __init__(
        self, callbacks: Optional[List[Callable[[BreedingEvent], None]]] = None
    ):
    ...
    def get_history(
        self, event_type: Optional[BreedingEventType] = None
    ) -> List[BreedingEvent]:
...
class BuildCoordinator:
    ...
    def __init__(
        ...
        self.consensus = FleetBreederConsensus(
            ...
            archive_dims=[len(manifest.qd_dimensions)]
            if manifest.qd_dimensions
            else [1],
            behavior_bounds=[(0, 1)]
            * (len(manifest.qd_dimensions) if manifest.qd_dimensions else 1),
        )
    ...
    def propose_generation(self, generation: int, parents: List[Any]) -> Dict[str, Any]:
        ...
        # Add BFT consensus
        result = self.consensus.propose_breeding_batch(
            candidates, batch_size=min(4, len(candidates))
        )
    ...
    def run_coordinated_breeding(
        ...
        for gen in range(generations):
            ...
            qd_score = None
            if (
                hasattr(self.local_breeder, "qd_archive")
                and self.local_breeder.qd_archive
            ):
        ...
        final_best = getattr(
            self.local_breeder,
            "best_fitness",
            getattr(self.local_breeder, "solver_best", 0.0),
        )
        pop_size = (
            len(self.local_breeder.population)
            if hasattr(self.local_breeder, "population")
            else 0
        )
        event = BreedingEvent(
...
class HarnessAdapter:
    ...
    def run_breeding(
        self, task_fn: Callable[[Any], float], generations: Optional[int] = None
    ) -> Iterator[BreedingEvent]:
    ...
    def _run_local_breeding(
        self, task_fn: Callable[[Any], float], generations: int
    ) -> Iterator[BreedingEvent]:
        ...
        final_best = getattr(
            breeder, "best_fitness", getattr(breeder, "solver_best", 0.0)
        )
    ...
    def get_best(self) -> Tuple[Any, float]:
        ...
        if hasattr(self, "_last_breeder") and self._last_breeder is not None:
            ...
            if self._last_breeder.population:
                return self._last_breeder.population[0], getattr(
                    self._last_breeder.population[0], "fitness", 0.0
                )

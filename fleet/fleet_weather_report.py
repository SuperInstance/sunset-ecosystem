    ...
    * Historical data for trends is pulled from conductor status logs
    ...
    def from_conductor(cls, conductor: Any) -> "FleetWeatherReport":
        ...
        self_health = NodeHealth(
            ...
            drift_ms=drift_ms,
            beat_sync="synced" if drift_ms is not None and drift_ms < 10.0 else "drifting",
        ...
        stats.notable_events = (
            stats.errors[:5] + stats.anomalies[:5] + stats.high_diversity_discoveries[:5]
        )
    ...
    def to_markdown(self) -> str:
        ...
        for nh in s.node_health:
            ...
            if nh.consecutive_failures:
                lines.append(
                    f"  - ⚠️ {nh.consecutive_failures} consecutive failures"
                )
            if nh.last_error:
        ...
        lines.append("")
        lines.append(
            f"*Report generated by FleetWeatherReport v1.0*"
        )
    ...
    def _calculate_trend(self) -> str:
        ...
        if current_rate is not None and last_week_rate is not None and last_week_rate > 0:
...
def _maybe_load_history(stats: FleetStats) -> None:
    ...
    try:
        ...
        one_week_ago = (
            stats.timestamp - timedelta(days=7)
        ).strftime("%Y-%m-%d")
        for entry in reversed(entries):

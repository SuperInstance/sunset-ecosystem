...
def forward_tensordot(w, x):
    ...
    # h = x @ w1[i]  ->  tensordot(w1, x, axes=([1],[0])) gives (n, 32)
    h = np.tensordot(w["w1"], x, axes=([1], [0])) + w["b1"][0]
    np.maximum(h, 0, out=h)
    # h[i] @ w2[i]  ->  tensordot(h, w2, axes=([1],[1])) ? No.
    # w2 is (n, 32, 16), h is (n, 32). We want output (n, 16).
    # tensordot(h, w2, axes=([1],[1])) gives (n, n, 16) — wrong.
    # Use einsum for the remaining layers.

...
Performs diversity search over the full agent population using the
FLUX constraint engine (novelty scoring + tournament results).
...
class FluxVectorTable:
    ...
    Diversity-search methods:
        - search_diverse_parents(n_results=2)
        - compute_diversity_matrix()
        - find_niche_centroids(k=3)
        - recommend_breed_pair()

...
class StrategyRecord:
    ...
    def update(self, improved: bool) -> None:
        ...
        if improved:
            self.successes += 1
        self.ema_rate = (
            self.alpha * (1.0 if improved else 0.0) + (1 - self.alpha) * self.ema_rate
        )
...
class MetaLearningBreeder:
    ...
    def fingerprint(
        self, genome: List[float], constraints: List[str], landscape: str = "unknown"
    ) -> ProblemFingerprint:
    ...
    def mutate(
        self, genome: List[float], fingerprint: ProblemFingerprint
    ...
    def get_strategy_stats(
        self, fingerprint: Optional[ProblemFingerprint] = None
    ) -> Dict:
        ...
        return {
            name: {"attempts": r.attempts, "successes": r.successes, "rate": r.ema_rate}

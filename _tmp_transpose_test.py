...
def _forward_einsum_numba_core_transposed(
    x, w1_t, b1, w2_t, b2, w3_t, b3, n,
):
    """w1_t is (n, 32, 64) so inner loop over k is contiguous."""
    out = np.empty((n, 16), dtype=np.float32)
    for i in range(n):
        # layer 1: x(64) dot w1_t[i](32,64)^T -> h(32)
        # which is: for j in 32: sum_k x[k] * w1_t[i, j, k]
        ...
        for j in range(32):
            if h[j] < 0.0:
                h[j] = 0.0
        # layer 2: h(32) dot w2_t[i](16,32)^T -> h2(16)
        ...
        for j in range(16):
            if h2[j] < 0.0:
                h2[j] = 0.0
        # layer 3: h2(16) dot w3_t[i](16,16)^T -> out[i](16)
        for j in range(16):
            s = 0.0
            for k in range(16):
                s += h2[k] * w3_t[i, j, k]
...
def _compiled_forward_transposed(w, x):
    ...
    # Pre-transpose weights for cache locality in inner loop
    w1_t = np.ascontiguousarray(w["w1"].transpose(0, 2, 1))  # (n, 32, 64)
    w2_t = np.ascontiguousarray(w["w2"].transpose(0, 2, 1))  # (n, 16, 32)
    w3_t = np.ascontiguousarray(w["w3"].transpose(0, 2, 1))  # (n, 16, 16)
    return _forward_einsum_numba_core_transposed(xflat, w1_t, b1, w2_t, b2, w3_t, b3, n)

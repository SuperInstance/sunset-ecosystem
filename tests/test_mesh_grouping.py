...
class TestKMeansClustering:
    def test_cluster_creates_groups(self, base_table: MeshVectorTable, sample_entries: list[VectorTableEntry]) -> None:
        for e in sample_entries:
            base_table.insert(e, skip_verify=True)
        grouping = MeshGrouping(base_table, config=ClusterConfig(algorithm="kmeans", n_clusters=4))
        groups = grouping.cluster()
        assert len(groups) >= 1
        assert all(isinstance(g, GroupProfile) for g in groups)

    def test_group_members(self, base_table: MeshVectorTable, sample_entries: list[VectorTableEntry]) -> None:
        for e in sample_entries:
            base_table.insert(e, skip_verify=True)
        grouping = MeshGrouping(base_table, config=ClusterConfig(algorithm="kmeans", n_clusters=4))
        grouping.cluster()
    ...
    def test_centroids(self, base_table: MeshVectorTable, sample_entries: list[VectorTableEntry]) -> None:
        for e in sample_entries:
            base_table.insert(e, skip_verify=True)
        grouping = MeshGrouping(base_table, config=ClusterConfig(algorithm="kmeans", n_clusters=4))
        grouping.cluster()
...
class TestIncrementalUpdate:
    def test_single_pass_online(self, base_table: MeshVectorTable) -> None:
        grouping = MeshGrouping(base_table, config=ClusterConfig(algorithm="single_pass", n_clusters=4))
    ...
    def test_incremental_outlier(self, base_table: MeshVectorTable) -> None:
        grouping = MeshGrouping(base_table, config=ClusterConfig(algorithm="single_pass", n_clusters=4, similarity_threshold=0.99))
        # First entry creates a group
        entry1 = VectorTableEntry(
            agent_id="a1", vector=np.array([1.0, 0.0], dtype=np.float32),
            timestamp=1000.0, node_id="test", generation=0, fitness=0.5, signature="test_signature_1",
        )
        base_table.insert(entry1, skip_verify=True)
        grouping.incremental_update(entry1)
        # Very different entry
        entry2 = VectorTableEntry(
            agent_id="a2", vector=np.array([100.0, 0.0], dtype=np.float32),
            timestamp=1000.0, node_id="test", generation=0, fitness=0.5, signature="test_signature_2",
        )
...
class TestQualityMetrics:
    def test_cohesion(self, base_table: MeshVectorTable, sample_entries: list[VectorTableEntry]) -> None:
        for e in sample_entries:
            base_table.insert(e, skip_verify=True)
        grouping = MeshGrouping(base_table, config=ClusterConfig(algorithm="kmeans", n_clusters=4))
        grouping.cluster()
    ...
    def test_separation(self, base_table: MeshVectorTable, sample_entries: list[VectorTableEntry]) -> None:
        for e in sample_entries:
            base_table.insert(e, skip_verify=True)
        grouping = MeshGrouping(base_table, config=ClusterConfig(algorithm="kmeans", n_clusters=4))
        grouping.cluster()
    ...
    def test_diversity_index(self, base_table: MeshVectorTable, sample_entries: list[VectorTableEntry]) -> None:
        for e in sample_entries:
            base_table.insert(e, skip_verify=True)
        grouping = MeshGrouping(base_table, config=ClusterConfig(algorithm="kmeans", n_clusters=4))
        grouping.cluster()
...
class TestDenseSparseRegions:
    def test_find_dense(self, base_table: MeshVectorTable, sample_entries: list[VectorTableEntry]) -> None:
        for e in sample_entries:
            base_table.insert(e, skip_verify=True)
        grouping = MeshGrouping(base_table, config=ClusterConfig(algorithm="kmeans", n_clusters=4))
        grouping.cluster()
    ...
    def test_find_sparse(self, base_table: MeshVectorTable, sample_entries: list[VectorTableEntry]) -> None:
        for e in sample_entries:
            base_table.insert(e, skip_verify=True)
        grouping = MeshGrouping(base_table, config=ClusterConfig(algorithm="kmeans", n_clusters=4))
        grouping.cluster()

...
class TestHolonomicBFT:
    ...
    def test_receive_vote(self):
        consensus = HolonomicBFT(node_id="alpha", peers=["beta"], f=0)
        vote = Vote(node_id="beta", proposal_id="batch_1", value=[0.5, 0.5], timestamp=0.0)
    ...
    def test_check_holonomy_with_quorum(self):
        ...
        for node in consensus.all_nodes:
            if node == "alpha":
                ...
                consensus.receive_vote(Vote(node_id=node, proposal_id="batch_1", value=[0.6, 0.8], timestamp=0.0))
    ...
    def test_get_holonomy_error(self):
        consensus = HolonomicBFT(node_id="alpha", peers=["beta"], f=0)
        consensus.propose("batch_1", value=[0.6, 0.8])
        consensus.receive_vote(Vote(node_id="beta", proposal_id="batch_1", value=[0.6, 0.8], timestamp=0.0))
    ...
    def test_is_byzantine_fault(self):
        ...
        consensus.propose("batch_1", value=[0.6, 0.8])
        consensus.receive_vote(Vote(node_id="beta", proposal_id="batch_1", value=[0.6, 0.8], timestamp=0.0))
        consensus.receive_vote(Vote(node_id="gamma", proposal_id="batch_1", value=[-0.6, -0.8], timestamp=0.0))
        consensus.receive_vote(Vote(node_id="delta", proposal_id="batch_1", value=[-0.6, -0.8], timestamp=0.0))

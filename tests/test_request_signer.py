...
class TestRequestSigner:
    ...
    def test_verify(self):
        signer = RequestSigner(secret="my-secret", clock=lambda: 0)
        signature = signer.sign("/api/users", method="POST", body=b"data")
        assert signer.verify("/api/users", method="POST", body=b"data", signature=signature) is True
    ...
    def test_verify_wrong_path(self):
        signer = RequestSigner(secret="my-secret", clock=lambda: 0)
        signature = signer.sign("/api/users", method="POST", body=b"data")
        assert signer.verify("/api/other", method="POST", body=b"data", signature=signature) is False

    def test_verify_wrong_method(self):
        signer = RequestSigner(secret="my-secret", clock=lambda: 0)
        signature = signer.sign("/api/users", method="POST", body=b"data")
        assert signer.verify("/api/users", method="GET", body=b"data", signature=signature) is False

    def test_verify_wrong_body(self):
        signer = RequestSigner(secret="my-secret", clock=lambda: 0)
        signature = signer.sign("/api/users", method="POST", body=b"data")
        assert signer.verify("/api/users", method="POST", body=b"tampered", signature=signature) is False

    def test_verify_with_headers(self):
        signer = RequestSigner(secret="my-secret", clock=lambda: 0)
        headers = {"content-type": "application/json"}
        signature = signer.sign("/api/users", method="POST", body=b"data", headers=headers)
        assert signer.verify("/api/users", method="POST", body=b"data", headers=headers, signature=signature) is True
    ...
    def test_ttl_expiration(self):
        ...
        assert signer.verify("/api/users", method="POST", body=b"data", signature=signature) is False

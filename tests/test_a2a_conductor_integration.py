...
class TestHandleBeatSyncTask:
    ...
    def test_large_drift_skip_jump(self, conductor):
        ...
        with patch.object(BeatSyncTask, "execute", return_value=A2AMetronomeResult(
            status="completed", task_type="beat_sync",
            payload={"delta_beats": 2.0, "delta_ns": 20_000_000, "beat_duration_ns": 500_000_000},
        )):
            task = BeatSyncTask(node_id="peer", target_beat=5, timestamp_ns=1000, bpm=120.0)
            result = handle_beat_sync_task(conductor, task)
            assert result.status == "completed"
    ...
    def test_large_drift_phase_nudge(self, conductor):
        ...
        with patch.object(BeatSyncTask, "execute", return_value=A2AMetronomeResult(
            status="completed", task_type="beat_sync",
            payload={"delta_beats": 0.5, "delta_ns": 20_000_000, "beat_duration_ns": 500_000_000},
        )):
            task = BeatSyncTask(node_id="peer", target_beat=5, timestamp_ns=1000, bpm=120.0)
            result = handle_beat_sync_task(conductor, task)
            assert result.status == "completed"
...
class TestHandleBPMProposalTask:
    def test_accept(self, conductor):
        with patch.object(BPMNegotiationTask, "execute", return_value=A2AMetronomeResult(
            status="completed", task_type="bpm_negotiation",
            payload={"action": "accept", "accepted_bpm": 140.0},
        )):
            task = BPMNegotiationTask(
                node_id="peer", proposed_bpm=140.0, ramp_ms=2000,
                reason="test", response_action="propose",
            )
    ...
    def test_reject(self, conductor):
        ...
        with patch.object(BPMNegotiationTask, "execute", return_value=A2AMetronomeResult(
            ...
            task = BPMNegotiationTask(
                node_id="peer", proposed_bpm=200.0, ramp_ms=2000,
                reason="test", response_action="propose",
            )
            result = handle_bpm_proposal_task(conductor, task)
            assert result.status == "completed"
...
class TestHandleDriftAlertTask:
    def test_skip_jump(self, conductor):
        with patch.object(DriftAlertTask, "execute", return_value=A2AMetronomeResult(
            status="completed", task_type="drift_alert",
            payload={"recommended_action": "skip_jump"},
        )):
            task = DriftAlertTask(
                node_id="peer", drift_ms=15.0, drift_beats=2.0,
                threshold_ms=10.0, requested_action="skip_jump", target_beat=5,
            )
    ...
    def test_partition(self, conductor):
        with patch.object(DriftAlertTask, "execute", return_value=A2AMetronomeResult(
            ...
            task = DriftAlertTask(
                node_id="peer", drift_ms=50.0, drift_beats=5.0,
                threshold_ms=10.0, requested_action="partition", target_beat=5,
            )
            result = handle_drift_alert_task(conductor, task)
            assert result.status == "completed"
    ...
    def test_phase_nudge(self, conductor):
        with patch.object(DriftAlertTask, "execute", return_value=A2AMetronomeResult(
            ...
            task = DriftAlertTask(
                node_id="peer", drift_ms=3.0, drift_beats=0.2,
                threshold_ms=10.0, requested_action="phase_nudge", target_beat=5,
            )
            result = handle_drift_alert_task(conductor, task)
            assert result.status == "completed"
    ...
    def test_no_action(self, conductor):
        with patch.object(DriftAlertTask, "execute", return_value=A2AMetronomeResult(
            ...
            task = DriftAlertTask(
                node_id="peer", drift_ms=0.5, drift_beats=0.01,
                threshold_ms=10.0, requested_action="phase_nudge", target_beat=5,
            )
            result = handle_drift_alert_task(conductor, task)
            assert result.status == "completed"
...
class TestHandleTickAsTask:
    def test_with_grid(self, conductor):
        ...
        with patch.object(TickAsTask, "execute", return_value=A2AMetronomeResult(
            status="completed", task_type="tick_as", payload={"tick": 1},
        )):
...
class TestFleetConductorA2AExtension:
    def test_dispatch_sync_task(self, conductor):
        register_a2a_handlers(conductor)
        payload = FleetConductorA2AExtension.dispatch_sync_task(conductor, "peer-1", target_beat=10)
        assert payload["type"] == "beat_sync"
        assert payload["input"]["target_beat"] == 10

    def test_dispatch_sync_task_no_scheduler(self):
        fc = FleetConductor("n", "http://test:4047")
        payload = FleetConductorA2AExtension.dispatch_sync_task(fc, "peer-1", target_beat=10)
        assert payload["type"] == "beat_sync"
    ...
    def test_dispatch_bpm_proposal(self, conductor):
        ...
        payload = FleetConductorA2AExtension.dispatch_bpm_proposal(
            conductor, "peer-1", proposed_bpm=140.0, ramp_ms=3000, reason="load",
        )
    ...
    def test_dispatch_drift_alert(self, conductor):
        ...
        payload = FleetConductorA2AExtension.dispatch_drift_alert(
            conductor, "peer-1", drift_ms=15.0, drift_beats=2.0, requested_action="skip_jump",
        )
    ...
    def test_dispatch_drift_alert_no_scheduler(self):
        fc = FleetConductor("n", "http://test:4047")
        payload = FleetConductorA2AExtension.dispatch_drift_alert(
            fc, "peer-1", drift_ms=15.0, drift_beats=2.0,
        )
        assert payload["type"] == "drift_alert"
    ...
    def test_handle_incoming_task_beat_sync(self, conductor):
        register_a2a_handlers(conductor)
        task_dict = BeatSyncTask(node_id="peer", target_beat=5, timestamp_ns=1000, bpm=120.0).to_dict()
        with patch.object(BeatSyncTask, "execute", return_value=A2AMetronomeResult(
            status="completed", task_type="beat_sync", payload={},
        )):
    ...
    def test_handle_incoming_task_unknown(self, conductor):
        register_a2a_handlers(conductor)
        result = FleetConductorA2AExtension.handle_incoming_task(conductor, {"type": "unknown"})
        assert result.status == "failed"
    ...
    def test_handle_incoming_task_no_handlers(self):
        ...
        _A2A_HANDLER_REGISTRY.clear()
        result = FleetConductorA2AExtension.handle_incoming_task(fc, {"type": "beat_sync"})
        assert result.status == "failed"

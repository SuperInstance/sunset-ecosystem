...
class TestCaslangScript:
    ...
    def test_from_task_graph(self) -> None:
        graph = {
            "tasks": [
                {"id": "t1", "action": "set_var", "params": {"name": "foo", "value": "bar"}},
                {"id": "t2", "action": "read_file", "params": {"path": "/tmp/test.txt", "as": "data"}},
            ],
...
class TestExecution:
    ...
    def test_json_parse(self) -> None:
        executor = CaslangExecutor()
        script = CaslangScript(commands=[
            {"op": "flow.set", "name": "raw", "value": '{"name":"test","value":42}'},
        ...
        assert "test" in str(result["output"]) or result["output"] == {"name": "test", "value": 42}
    ...
    def test_rollback_on_failure(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            script = CaslangScript(commands=[
                ...
                {"op": "fs.read_file", "path": "/nonexistent/file.txt"},  # will fail
            ])
            result = executor.execute(script)
            assert result["status"] in ("failed", "validation_failed")
            assert result.get("rollback", False) is True or result["status"] == "validation_failed"
...
class TestStats:
    def test_stats_tracking(self) -> None:
        ...
        # One failure
        bad_script = CaslangScript(commands=[
            {"op": "fs.read_file", "path": "/etc/passwd"},
        ])

...
class TestRetryHandler:
    ...
    def test_retry_then_success(self):
        ...
        retry = RetryHandler(
            RetryPolicy(
                max_attempts=3, base_delay=0.01, on_exceptions=(ConnectionError,)
            )
        )
    ...
    def test_exhausted(self):
        retry = RetryHandler(
            RetryPolicy(max_attempts=2, base_delay=0.01, on_exceptions=(ValueError,))
        )
    ...
    def test_no_retry_for_unexpected_exception(self):
        retry = RetryHandler(
            RetryPolicy(max_attempts=3, on_exceptions=(ConnectionError,))
        )
    ...
    def test_retry_condition(self):
        ...
        retry = RetryHandler(
            RetryPolicy(max_attempts=5, base_delay=0.01, retry_if=lambda x: x is None)
        )
    ...
    def test_retry_condition_exhausted(self):
        retry = RetryHandler(
            RetryPolicy(max_attempts=2, base_delay=0.01, retry_if=lambda x: x is None)
        )
        with pytest.raises(RetryExhausted):

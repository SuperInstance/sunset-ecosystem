...
if __name__ == "__main__":
    ...
    for n in [100, 250, 500, 1000]:
        ...
        diff = np.max(np.abs(forward_einsum(w, x) - _forward_einsum_numba_core_parallel(x, w1, b1, w2, b2, w3, b3, n)))
        print(f"n={n:4d}  numba={t_numba/trials:.3f}ms  numpy={t_numpy/trials:.3f}ms  speedup={t_numpy/t_numba:.2f}x  diff={diff:.2e}")
